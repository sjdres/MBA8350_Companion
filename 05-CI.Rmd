...
Suppose there exists a bowl of numbered cards.
The numbers on these cards comprises a normal distribution where the mean value is 5, and the standard deviation is 3.
$$X \sim N(5,3)$$
...
1. Using the *default* setting: suppose you want to calculate all of the probabilities using the default setting of calculating areas to the left. The gray slice of the distribution is then the difference between the area to the left of $-\frac{2}{3}$ and the area to the left of -1.
...
2. Removing the *default* setting. If you want to calculate probabilities from the right (which might come in handy), then the same slice of the distribution is then the difference between the area to the right of $-1$ and the area to the right of $-\frac{2}{3}$.
...
(n = (Z*Sig/e)^2)
# Round up since you can have a fraction of an observation
ceiling(n)
```
